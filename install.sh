...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red}mistake: ${plain} This script must be run as root!\n" && exit 1
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/mostafaprohub/x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}Failed to detect the x-ui version. It may have exceeded the Github API limit. Please try again later, or manually specify the x-ui version for installation.${plain}"
            exit 1
        fi
        echo -e "Latest version of x-ui detected: ${last_version}，Start installation"
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/mostafaprohub/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/mostafaprohub/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/mostafaprohub/x-ui/main/x-ui.sh
    ...
    echo -e "${green}x-ui v${last_version}${plain} The installation is complete and the panel has been started. "
    echo -e ""
    ...
    echo -e "x-ui disable      - Cancel x-ui auto-start at boot"
