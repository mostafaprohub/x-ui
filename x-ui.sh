...
set_port() {
    echo && echo -n -e "Enter port number[1000-65535]: " && read port
    if [[ -z "${port}" ]]; then
        LOGD "Canceled"
        before_show_menu
    else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/mostafaprohub/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_enable_status() {
    check_enabled
    if [[ $? == 0 ]]; then
        echo -e "Whether to start automatically at boot: ${green}yes${plain}"
    else
...
————————————————
  ${green}15.${plain} 一Key installation bbr (latest kernel)
