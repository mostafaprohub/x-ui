...
func (s *Server) initI18n(engine *gin.Engine) error {
	...
	findI18nParamNames := func(key string) []string {
		...
		for i := 0; i < keyLen-1; i++ {
			if key[i:i+2] == "{{" { // Beginning of judgment "{{"
				j := i + 2
				isFind := false
				for ; j < keyLen-1; j++ {
					if key[j:j+2] == "}}" { // ending "}}"
...
func (s *Server) startTask() {
	...
	// Checks every 30 seconds to see if xray is running
	s.cron.AddJob("@every 30s", job.NewCheckXrayRunningJob())

	go func() {
		time.Sleep(time.Second * 5)
		// Traffic is counted every 10 seconds, and the first startup is delayed by 5 seconds, staggered with the time to restart xray
		s.cron.AddJob("@every 10s", job.NewXrayTrafficJob())
	}()

	// Checks inbound traffic for excess and expiration every 30 seconds
	s.cron.AddJob("@every 30s", job.NewCheckInboundJob())
	// Traffic status will be prompted once a day, at 8:30 Shanghai time
	var entry cron.EntryID
	isTgbotenabled, err := s.settingService.GetTgbotenabled()
	if (err == nil) && (isTgbotenabled) {
		runtime, err := s.settingService.GetTgbotRuntime()
		if err != nil || runtime == "" {
			logger.Errorf("Add New Stats Notify Job error[%s],Runtime[%s] invalid,wil run default", err, runtime)
			runtime = "@daily"
		}
		logger.Infof("Tg notify enabled,run at %s", runtime)
		entry, err = s.cron.AddJob(runtime, job.NewStatsNotifyJob())
		if err != nil {
			logger.Warning("Add New Stats Notify Job error", err)
			return
		}
	} else {
		s.cron.Remove(entry)
